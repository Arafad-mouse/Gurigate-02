...
import { Calendar, Clock, User, Phone, Mail, CheckCircle, DollarSign } from 'lucide-react';
...
export default function TodayCheckOutsPage() {
  ...
  useEffect(() => {
    // TODO: Fetch today's check-outs from API
    setLoading(false);
  }, []);
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Manage guest departures for today</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {checkOuts.map((checkOut) => (
          <div key={checkOut.id} className="bg-white rounded-lg shadow p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="font-semibold text-gray-900">{checkOut.guestName}</h3>
                <p className="text-sm text-gray-600">{checkOut.propertyTitle}</p>
              </div>
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                checkOut.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
              }`}>
