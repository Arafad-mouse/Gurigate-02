...
export class GuriGatePropertyService {
  ...
  static async searchProperties(query: string, filters?: {
    ...
    try {
      ...
      // Apply badge filter after conversion
      if (filters?.badge) {
        properties = properties.filter(p => p.badge === filters.badge);
      }
  ...
  static async getPropertyById(id: string): Promise<LandingProperty | null> {
    try {
      ...
      const flattenedData = {
        ...
        primary_image_url: data.property_images?.find((img: any) => img.is_primary)?.url,
